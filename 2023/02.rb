INPUT = DATA.read.lines(chomp: true).map { |line|
  id = line.split(?:).first.split.last.to_i
  sets = line.split(": ").last.split("; ").map { |set|
    set.split(", ").map { |cubes|
...
p INPUT.map { |_, game|
  ...
  game.each { |set|
    set.each { |count, color|
      minimums[color] = [minimums[color], count].max
    }
  }
