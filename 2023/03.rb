# Not cleaning this up lmao

INPUT = DATA.read.strip.lines(chomp: true).map(&:chars)

SCHEMATIC = Hash.new(nil)

INPUT.each_with_index { |line, y|
  line.each_with_index { |c, x|
    SCHEMATIC[[x, y]] = c unless c == "."
  }
}

potential_gears = Hash.new(nil)
gear_ratios = 0

sum = 0
SCHEMATIC.each { |(x, y), c|
  next if c.nil? || !c.match(/\d/) || SCHEMATIC[[x-1, y]]&.match?(/\d/)
  ...
  (x..).each { |x_|
    value = SCHEMATIC[[x_, y]]
    break if value.nil? || !value.match?(/\d/)
    number *= 10
    number += SCHEMATIC[[x_, y]].to_i
    last_x = x_
  }

  is_part = (y-1..y+1).any? { |y_|
    (x-1..last_x+1).any? { |x_|
      if SCHEMATIC[[x_, y_]] == "*"
        if potential_gears[[x_, y_]]
          gear_ratios += number * potential_gears[[x_, y_]]
        else
          potential_gears[[x_, y_]] = number
        end
      end
      !SCHEMATIC[[x_, y_]].nil? && !SCHEMATIC[[x_, y_]].match?(/\d/)
    }
  }

  sum += number if is_part
}
