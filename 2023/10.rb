# I was struggling with this part as well, until I stumbled upon Pick's Theorem while googling. The b/2 is what was solved in part 1. Calculate the area and solve for i, then you have the result.
# Shoelace formula
...
INPUT.lines(chomp: true).each.with_index { |line, y|
...
[[-1, 0], [1, 0], [0, -1], [0, 1]].each { |dx, dy|
  ...
  if MAP[neighbor]&.include?(relation)
    MAP[start] <<= [dx, dy]
  end
}

distance = 0
current = [start[0] + MAP[start][0][0], start[1] + MAP[start][0][1]]
prev = start
main_loop = Hash.new
main_loop[start] = true

while current != start
  main_loop[current] = true
  #puts "current=#{current}"
  MAP[current].each { |dx, dy|
    x = current[0] + dx
    y = current[1] + dy
...
min_x, max_x = main_loop.keys.map(&:first).minmax
min_y, max_y = main_loop.keys.map(&:last).minmax
...
y_range.each { |y|
  is_inside = false
  x_range.each { |x|
    if main_loop[[x, y]] && MAP[[x, y]].sort != [[-1, 0], [1, 0]]
      is_inside = !is_inside
    end

    insides[[x, y]] = is_inside
  }
}
...
y_range.each { |y|
  x_range.each { |x|
    ...
    if main_loop[[x, y]]
      print INPUT.lines(chomp: true)[y][x]
    else
      print '.'
    end
