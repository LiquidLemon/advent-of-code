...
def deduce(connections)
  LETTERS.permutation { |perm|
    pad = LETTERS.zip(perm).to_h

    translated = connections.map { |x|
      x.chars.map { |c| pad[c] }.sort.join
    }

    if translated.all? { |x| DIGITS.include?(x) }
      return pad
    end
  }

  raise "No matching pad found."
end

puts input.sum { |connections, display|
  pad = deduce(connections)
  translated = display.map { |digit|
    digit.chars.map { |c| pad[c] }.sort.join
  }

  raise "incorrect translation" if translated.any? { |x| !DIGITS.include?(x) }

  translated.reverse.each_with_index.reduce(0) { |acc, (key, i)|
    acc + DIGITS[key] * 10 ** i
  }
}
