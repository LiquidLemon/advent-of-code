...
count_orbits = ->(start='COM', depth=0) {
  return depth unless orbits[start]

  depth + orbits[start].sum do |orbiter|
    count_orbits.(orbiter, depth + 1)
  end
}
...
distance = you.length + san.length - 2 * (common_length + 1)
