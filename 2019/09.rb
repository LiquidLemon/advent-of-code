class Interpreter
  ...
  def initialize(program, input=[])
    @memory = program.length.times.zip(program).to_h
    @memory.default = 0
    @input = input
  end
  ...
  def run
    ...
    Enumerator.new do |y|
      loop do
        ...
        args = (ip + 1..ip + arity).map { |x| @memory[x] }
        modes = opcode.drop(2)
        ...
        set_memory = proc do |i, value|
          case modes[i].to_i
          when 0
            @memory[args[i]] = value
          when 1
            raise "Trying to write to immediate parameter"
          when 2
            @memory[args[i] + base] = value
          end
        end
        ...
        case index
        when 1
          set_memory.(2, get_param.(0) + get_param.(1))
        when 2
          set_memory.(2, get_param.(0) * get_param.(1))
        when 3
          set_memory.(0, @input.shift)
        when 4
          y << get_param.(0)
        when 5
          if get_param.(0) != 0
        ...
        when 7
          set_memory.(2, if get_param.(0) < get_param.(1) then 1 else 0 end)
        when 8
          set_memory.(2, if get_param.(0) == get_param.(1) then 1 else 0 end)
        when 9
          base += get_param.(0)
...
boost = DATA.read.split(?,).map(&:to_i)
int = Interpreter.new(boost, [2])

puts "Part 1: #{int.run.to_a}"
