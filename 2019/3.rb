...
wires = DATA.readlines.map do |line|
  line.split(',').map do |step|
    dir = step[0]
    length = step.chars.drop(1).join.to_i
...
wires.each.with_index do |wire, wire_i|
  location = [0, 0]
  length = 0

  wire.each do |pos|
    pos.each.with_index do |coord, i|
      dir = coord <=> 0

      coord.abs.times do
        length += 1
        location[i] += dir
        occupied[location.dup] ||= Hash.new
        occupied[location.dup][wire_i] = length unless occupied[location.dup][wire_i]
      end
    end
  end
end

crossing = occupied.select do |pos, wires|
  wires.size > 1
end

closest = crossing.keys.min_by { |x| distance([0, 0], x) }

puts "Part 1: #{distance([0, 0], closest)}"

least_steps = crossing.values.map { |x| x.values.sum }.min

puts "Part 2: #{least_steps}"
