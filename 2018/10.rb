require 'set'
input = DATA.readlines.map { |l| l.scan(/-?\d+/).map(&:to_i) }

def display(points)
  x_range = Range.new(*points.map { |p| p[0] }.minmax)
  y_range = Range.new(*points.map { |p| p[1] }.minmax)
  set = Set.new(points.map { |x, y, _, _| [x, y] } )
  y_range.each do |y|
    x_range.each do |x|
