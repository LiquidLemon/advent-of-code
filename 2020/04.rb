data = DATA.read.split("\n\n").map { |entry|
  entry.split(/\s+/).map { |kv| kv.split(?:) }.to_h
}

valid = data.filter { |entry| (entry.keys - ["cid"]).size == 7 }
puts valid.count

puts valid.count { |entry|
  [
    (1920..2002).include?(entry["byr"].to_i),
    (2010..2020).include?(entry["iyr"].to_i),
    (2020..2030).include?(entry["eyr"].to_i),
    entry["hgt"][-2..-1] == "cm" && (150..193).include?(entry["hgt"][...-2].to_i) ||
    entry["hgt"][-2..-1] == "in" && (59..76).include?(entry["hgt"][...-2].to_i),
    ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].include?(entry["ecl"]),
    entry["hcl"] =~ /^#[0-9a-f]{6}$/,
    entry["pid"] =~ /^\d{9}$/
  ].all?
}
