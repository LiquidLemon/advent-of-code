# Part 1 could be done more efficiently by building the (multi)tree in reverse
# but the forward way is still better for part 2. It's possible that using
# a dedicated structure for each would be more efficient.

data = DATA.each_line.map { |l|
  bag_color, rest = l.split(' bags contain ')
  bags = rest.split(', ').map { |b|
    count, *color, _ = b.split
    [count.to_i, color.join(' ')]
  }
  bags = [] if bags == [[0, 'other']]
  [bag_color, bags]
}.to_h

contains = ->(color) {
  data[color].map(&:last).include?('shiny gold') ||
  data[color].map(&:last).any?(contains)
}

puts data.count { |color, _| contains.(color) }

count = ->(color) {
  data[color].sum { |amount, color|
    amount * (count.(color) + 1)
  }
}
